...
//     List<n_st> getByIINID(String IINID);
    ...
    @Query("MATCH (n:n_st) WHERE n.IINID=~('(?i).*'+'(?i)'+($IINID)+'.*') or n.FIO=~('(?i).*'+'(?i)'+($IINID)+'.*') CALL apoc.path.subgraphAll(n, {relationshipFilter: ($REL)}) YIELD nodes, relationships WHERE size(nodes) > 1  UNWIND nodes as node  MATCH (node)-[r]->(u)  return node, r, u")
    ...
    @Query("MATCH (u:node_c)<-[r:rel_final]-(m:n_st)  WHERE  date({day: [item in split(r.start_date, \".\") | " +
            "toInteger(item)][0], month: [item in split(r.start_date, \".\") | " +
            "toInteger(item)][1], year: [item in split(r.start_date, \".\") | toInteger(item)][2]}) " +
            " <=  date(($start_date))  and  (date({day: [item in split(r.end_date, \".\") | toInteger(item)][0]," +
            " month: [item in split(r.end_date, \".\") | toInteger(item)][1], year:" +
            " [item in split(r.end_date, \".\") | toInteger(item)][2]}) >=  date(($end_date)) or " +
            "r.end_date= '') and (u.BINID=~($BINID) or u.company =~('(?i).*'+'(?i)'+($BINID)+'.*')) return m,u,r")
